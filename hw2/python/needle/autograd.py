...
class Tensor(Value):
    ...
    def data(self, value):
        ...
        if value.dtype in ["float64", "float32"] and self.dtype in ["float64", "float32"]:
            pass
        else:
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
        adjoint_i = sum_node_list(node_to_output_grads_list[i])
        i.grad = adjoint_i
        if i.op is None:
            continue
        partial_grad = i.op.gradient_as_tuple(adjoint_i, i)
        for k, grad_k in zip(i.inputs, partial_grad):
            if k not in node_to_output_grads_list:
                node_to_output_grads_list[k] = []
            node_to_output_grads_list[k].append(grad_k)
